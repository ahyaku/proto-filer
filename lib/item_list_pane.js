...
class ItemListPane{
  ...
  set dir_cur(dir_cur){
//    try{
//      let stat = fs.access(dir_cur, fs.constants.R_OK | fs.constants.W_OK);
//    }catch(e){
//      console.log('dir_cur <> catch: ' + e);
//    }
...
//  }
  ...
  updateItems(){
    ...
    //for(let e of fs.readdirSync(this._dir_cur)){
    //  item = new Item();
    //  item.name = e;
  ...
  _drawItems(){
    ...
    //node_id.setAttribute('style', 'background-color: rgba(0, 0, 0, 1);');

    //node_id.documentElement.style.overflowY = 'auto';
    ...
    */
    ...
    //if(this._line_cur >= this._items.length){
    //  console.log("ERROR!!: line_cur >= _item.length");
    //  console.log('line_cur = ' + this._line_cur);
    //  console.log('items.length = ' + this._items.length);
    ...
    for(let e of this._items){
      ...
      //node.style.cssText = 'background: rgba(0, 0, 255, .1)';
      //node.setAttribute('background', 'rgba(0, 0, 255, .1');
      if(e.is_dir){
        node.setAttribute('style', 'color:#FFFFFF;');
      }else{
        node.setAttribute('style', 'color:#00FF00;');
      }
      //node.setAttribute('style', 'background-color: rgba(0, 0, 255, .1);');
      node_frg.appendChild(node);
    }
    node_id.appendChild(node_frg);

    //console.log('document.body.scrollHeight: ' + document.body.scrollHeight);
    //console.log('node_id.scrollHeight: ' + node_id.scrollHeight);
    //window.scrollTo(0, node_id.scrollHeight);
  ...
  drawCursor(cn){
    ...
    //let cc = (this._line_prv < this._items.length) ? this._line_prv : this._items.length-1;
    if( cn < 0 ){
      cn = this._items.length - 1;
    }else if( cn >= this._items.length ){
      cn = 0;
    }
    ...
    if(this._enabled == true){
      e[cn].style.borderBottomWidth = 'thin';
      e[cn].style.borderBottomStyle = 'solid';
      e[cn].style.borderBottomColor = '#FFFFFF';
